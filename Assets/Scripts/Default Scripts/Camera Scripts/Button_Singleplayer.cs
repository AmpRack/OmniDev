...
public class Button_Singleplayer : MonoBehaviour {
	...
	void DoMoving(){
		...
		if ((letsMove == true) && (buttons.posNow != buttons.posSP)) {
			buttons.posNow = Vector3.MoveTowards(buttons.posNow, buttons.posSP, (buttons.moveSpeed * Time.deltaTime));
		}
		
		if ((letsMove) && (buttons.posNow == buttons.posSP)) {
			letsMove = false;
		}
	}
}